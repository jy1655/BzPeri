...
The build-time default can be changed with `-DBZP_DEFAULT_GLIB_LOG_CAPTURE_MODE=AUTOMATIC|DISABLED|HOST_MANAGED|STARTUP_AND_SHUTDOWN`, and `bzpGetConfiguredGLibLogCaptureMode()` exposes that compiled-in default at runtime.
...
The bundled `bzp-standalone` sample can be launched in this mode with `--manual-loop`, and its GLib capture strategy can be exercised with `--glib-log-capture=auto|off|host|startup-shutdown`.
