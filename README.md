...
The bundled `bzp-standalone` sample can be launched in this mode with `--manual-loop`, and its GLib capture strategy can be exercised with `--glib-log-capture=auto|off|host|startup-shutdown` plus `--glib-log-targets=all|log|log,printerr`.
